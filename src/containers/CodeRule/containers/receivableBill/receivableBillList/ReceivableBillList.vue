...
<section class="container">
    <div class="billing-btn-group">
        ...
        <el-button size="small" :plain="true" type="primary" v-show="!isFirstRender" :disabled="noSelectedData" @click="handleScan(selectionDataId)">刷新</el-button>
    </div>
        ...
        <el-table ref="multipleTable" :data="invoiceListData" border tooltip-effect="light" style="width: 100%" @selection-change="handleSelectionChange" @row-click="handleRowClick" highlight-current-row>
            ...
            <el-table-column header-align="center" v-for="(item, index) in cols" align="left" min-width="100" :prop="item.prop" :label="item.label" :key="index" show-overflow-tooltip>
                <template slot-scope="scope">
                        ...
                        <span v-else-if="item.prop === 'billStatus'" :title="item.label">{{ scope.row[item.prop] | invoiceStatusFormat }}</span>
...
<script>
// components
import searchDialog from '@CodeRule/components/SearchDialog.vue'
import moreSearch from '@CodeRule/components//MoreSearchDialog.vue'
// service
    ...
    methods: {
        ...
        handleModify() {
            ...
            this.$store.commit('Billing_M_ADD_HANDLE_DATA', {
                receivableBillIds: [this.operationData.id]
            })
        ...
        handleRowClick(row, event, column) {
            ...
            this.receivableBillOperationRowId = {
                payInfoDetailIdList: [row.id]
            }
                ...
                this.secondComfirmPop('此操作将永久删除数据，是否继续？', '删除')
                    .then(async () => {
                        let response = await paymentBillService.deleteById({
                            id: selection.payInfoDetailIdList[0]
                        })
                        // 处理返回结果
                ...
                this.secondComfirmPop('确定要提交吗？', '提交')
                    .then(async () => {
                        let response = await paymentBillService.submitById({
                            payInfoDetailIdList: selection.payInfoDetailIdList
                        })
        ...
        async searchReceivableBillList(searchCondition) {
            ...
            if (searchCondition) {
                searchData = Object.assign({}, searchData, {
                    conditions: this.searchDialogData.tableData
                })
            }
