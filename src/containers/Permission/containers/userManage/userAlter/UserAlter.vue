    ...
    <div class="user-add">
        ...
        <div class="addListForm-wrapper">
            <el-form :inline="true" :model="formData" class="demo-form-inline" label-width="100px" :rules="rules">
                ...
                <el-form-item label="手机号码" prop="phone">
                    <el-input v-model.number="formData.phone" placeholder="请输入" size="small" style="width: 160px;" :maxlength="11" :minlength="11"></el-input>
                </el-form-item>
            </el-form>
            <el-form :inline="true" :model="formData" class="demo-form-inline" label-width="100px" :rules="rules">
                <el-form-item label="邮箱地址" prop="email">
                    <el-input v-model="formData.email" placeholder="系统自动编码" size="small" style="width: 160px;"></el-input>
                </el-form-item>
            ...
            <el-form :inline="true" :model="formData" class="demo-form-inline" label-width="100px">
                ...
                <el-form-item label=" " label-width="30px">
                    <el-checkbox v-model="formData.lock" :disabled="true">锁定</el-checkbox>
                </el-form-item>
            </el-form>
            <el-form :inline="true" :model="formData" class="demo-form-inline" label-width="100px">
        ...
        <div class="addPermission-layer">
            <el-dialog title="添加功能权限" :visible.sync="perLayerMsg.addPerLayer" size="tiny" style="top: 20%;">
                <div class="tree-wrapper">
                    <fw-smarter-tree class="filter-tree" :enable-check="true" child-key="childFuncs" :expand-all="true" :data="model1" checked-key="check" tree-key="permissionId" prop="resourceName" ref="smartTree">
                    </fw-smarter-tree>
                </div>
                <span slot="footer" class="dialog-footer">
...
import { getRegisterSource, initialOrg, getRoleTree, getRolePermission, getUserFuncTree, getUserOrgTree, insertUser, getUserInfo, getUserFuncList, getUserOrgList, getUserDataLis, userRoleList, updateUser } from '@Permission/services/userService.js'
...
export default {
    ...
    methods: {
        ...
        saveUser () {
            let funcObj = []
            let orgIds = []
            console.log(this.$refs.funcTreeRef.getAllNodes(), 33333333333)
            this.$refs.funcTreeRef.getAllNodes().forEach(value => {
                funcObj.push({
                    authTime: value.authorTime ? value.authorTime : new Date().getTime(),
                    author: value.author ? value.author : USERID,
                    permissionId: value.permissionId
                })
            })
            this.orgTableData.forEach(value => {
                orgIds.push({
                    authTime: value.authorTime ? value.authorTime : new Date().getTime(),
                    author: value.author,
                    permissionId: value.permissionId
            ...
            console.log(this.orgTableData, 'this.orgTableData--------<<<<')
            for (var i = funcObj.length - 1; i >= 0; i--) {
                console.log(funcObj[ i ])
                if (funcObj[ i ].permissionId === 0.1 || funcObj[ i ].permissionId === 0.2) {
                    funcObj.splice(i, 1)
                }
            }
            this.roleTableData.forEach(value => {
                value.authTime = parseInt(value.authorTime)
            })
            console.log(funcObj, orgIds, 444)
            let ajaxData = {
                loginUserId: USERID,
                updateUser: USERID,
        ...
        cancelSaveUser () {
            if (this.newUserMsg.tabType === 'add') {
        ...
        addPermission () { // 添加权限
            this.getUserFuncTree({ LoginUserId: USERID, userId: this.newUserMsg.userId, groupId: GROUPID, chooseResourceName: "" })
        ...
        confirmAddOrg () {
            ...
            console.log(this.orgTreeArr, '-------<<this.orgTreeArrthis.orgTreeArr')
            this.orgTreeArr.forEach(value => {
                if(value.author === null) {
                    value.author = USERID
                }
                if(value.authorTime === null) {
