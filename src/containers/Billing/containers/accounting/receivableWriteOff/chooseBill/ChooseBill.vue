...
export default {
    ...
    methods: {
        ...
        handleWriteOff() {
            ...
            // TODO:  新建"手动核销-详情"页签
            // TODO:  新页签  tag 应该不相同（可能多个核销操作）
            this.addTab("应收核销-核销", 'receivable-write-Off', '', 'write-off')
        },
        // 模拟核销 按钮 事件
        ...
        async searchPayableBillList(searchCondition) {
            ...
            try {
                ...
                if (response.data.status.statusCode === 0) {
                    ...
                    this.selfTableData = response.data.result.list
                    // 关闭弹窗
        ...
        addTab(name, tag, tabType, content) {
            this.$root.bus.$emit('receivableWriteOff', name, tag, tabType, content)
        }
    },
    watch: {
        tabState: function() {
