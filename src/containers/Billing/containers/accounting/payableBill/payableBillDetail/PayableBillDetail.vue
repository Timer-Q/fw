...
<section class="invoicing billing-form-inline">
    <div class="billing-btn-group">
        ...
        <el-button size="small" :plain="true" type="primary" @click="handlePay" :disabled="billStatus !== 'Approve'" v-if="isScan">付款</el-button>
        <el-button size="small" :plain="true" type="primary" @click="handleAbandon" :disabled="billStatus !== 'Approve'" v-if="isScan">弃审</el-button>
        <!-- 操作 下拉框 -->
        <el-dropdown @command="handleCommand" v-if="isScan">
            ...
            <el-dropdown-menu class="billing-dropdown-menu" slot="dropdown">
                <el-dropdown-item command="invalid" :disabled="billStatus !== 'Approve'">作废</el-dropdown-item>
                <el-dropdown-item command="redRush" :disabled="billStatus !== 'Approve'">红冲</el-dropdown-item>
        ...
        <el-dropdown @command="handleUnionSearchCommand" v-if="isScan">
            ...
            <el-dropdown-menu class="billing-dropdown-menu" slot="dropdown">
                <el-dropdown-item command="searchCustomerStatus">查询付款单</el-dropdown-item>
                <el-dropdown-item command="searchReceipt">采购发票</el-dropdown-item>
                <el-dropdown-item command="searchUpstreamData">查询执行情况</el-dropdown-item>
            </el-dropdown-menu>
        </el-dropdown>
        <!-- 修改 -->
        <el-button size="small" :plain="true" type="primary" v-if="isModify || !isHasSource" @click="handleSave">保存</el-button>
        <el-button size="small" :plain="true" type="primary" v-if="!isHasSource" @click="handleSaveAndNew">保存并新增</el-button>
        <!-- 公共按钮 -->
        ...
        <el-button size="small" :plain="true" type="primary" v-if="isHasSource" @click="handleGetLog">日志</el-button>
    </div>
    ...
    <el-form :inline="true" :model="formData" :rules="rules" ref="payableBillDetalForm">
        ...
        <el-form-item label="往来对象类型" prop="objectType">
            <el-select v-model="formData.objectType" disabled placeholder="请选择往来对象类型">
                <el-option
        ...
        <el-form-item label="应付供应商" prop="payableObject">
            <el-input v-model="formData.payableObject" disabled placeholder="请输入应付供应商"></el-input>
        </el-form-item>
...
export default {
    ...
    data () {
        // 应付组织 校验
        var validatePayableOrg = (rule, value, callback) => {
            if (value === '') {
                callback(new Error('请输入应付组织'))
            } else if (!/^\d{1,7}$/.test(value)) {
                callback(new Error('请输入正确应付组织'))
            } else {
                callback()
            }
        }
    ...
    methods: {
        ...
        async handleInvalid () {
            this.$prompt('请输入作废原因', '提示', {
                ...
                inputValidator: this.validatePrompt
            }).then(async ({ value }) => {
                try {
        ...
        // 日志按钮操作
        handleGetLog () {
            this.addTab('日志', `${this.formData.id}-log`, '', 'get-log')
        },
        // '操作'下拉框
