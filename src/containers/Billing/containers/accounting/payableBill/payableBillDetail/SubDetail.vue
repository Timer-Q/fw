            ...
            <el-table-column header-align="center" v-for="(item, index) in cols" align="left" min-width="100" :prop="item.prop" :label="item.label" :key="index" show-overflow-tooltip>
                <!-- <el-table-column header-align="center" v-for="(each, index) in item.children" align="left" min-width="100" :prop="each.prop" :label="each.label" :key="index" show-overflow-tooltip> -->
                    <template slot-scope="scope">
                        <!-- select + popove -->
                        <el-popover
                        ref="goodsChargeCodePopover"
                        placement="bottom"
                        trigger="click"
                        popper-class="billing-department-popover"
                        v-if="item.prop === 'goodsChargeCode' && !isScan">
                            <!-- <el-select
                            ...
                            <el-input slot="reference" readonly :disabled="isScan" v-model="scope.row[item.prop]" placeholder="请选择内容"></el-input>
                        </el-popover>
                        <!-- goodsChargeCode scan 状态下兼容 -->
                        <el-input v-else-if="item.prop === 'goodsChargeCode' && isScan" :disabled="isScan" v-model="scope.row[item.prop]" placeholder="请选择内容"></el-input>                        
                        <!-- checkbox -->
                        <el-checkbox v-else-if="item.isCheckbox" :disabled="isScan || !item.isModified" v-model="scope.row[item.prop]"></el-checkbox>
                        <!-- input -->
                        <el-input v-else-if="item.isModified && item.prop !== 'goodsChargeCode'" :disabled="isScan" v-model="scope.row[item.prop]" placeholder="请输入内容">
                        </el-input>
                        <!-- link -->
                        <a href="javascript:;" v-else-if="(item.prop === 'sourceDocNum' || item.prop === 'originalDocNum' || item.prop === 'payContract') && !item.isModified">
                            {{ scope.row[item.prop] }}
                        </a>
                        <!-- normal -->
