...
<section class="container">
    <div class="billing-btn-group">
        <el-button size="small" :plain="true" type="primary" @click="handleSearchPayableBill">查询未确认应付单</el-button>
    ...
    methods: {
        handleCommand (command) {
            ...
            if (command === 'homemadePayableBill') {
                this.addTab(`自制应付单`, 'homemade-payable-bill', 'new', 'unconfirmed-payable-bill-detail')
                return
            }
        ...
        handleModify () {
            ...
            this.addTab(`${this.operationData.code}-修改`, `payableBill-${this.operationData.code}-edit`, 'modify', 'unconfirmed-payable-bill-detail')
        },
        // 显示详情
        handleShowDetails () {
            this.$store.commit('Billing_M_ADD_HANDLE_DATA', { payableBillIds: [ this.operationData.payableId ] })
            this.addTab(`${this.operationData.code}-查看`, `payableBill-${this.operationData.payableId}-scan-payableBillData`, 'scan', 'unconfirmed-payable-bill-detail')
        },
        // 点击table每一行
        ...
        addTab (name, tag, tabType, content) {
            this.$root.bus.$emit('unconfirmedPayableBill', name, tag, tabType, content)
        }
    },
    watch: {
        tabState () {
