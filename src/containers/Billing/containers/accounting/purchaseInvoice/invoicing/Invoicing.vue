<template>
  <section class="invoicing billing-form-inline">
    <div class="billing-btn-group">
      ...
      <el-button size="small" :plain="true" type="primary" v-if="!isScan" @click="handleSaveAndNew">保存并新增</el-button>
      ...
      <el-button size="small" :plain="true" type="primary" :disabled="multipleData.data.length <= 1" v-if="isScan">上一个</el-button>
      <el-button size="small" :plain="true" type="primary" :disabled="multipleData.data.length <= 1" v-if="isScan">下一个</el-button>
    ...
    <el-form :inline="true" :model="formData" :rules="VATComputed" ref="invoiceForm">
      <el-form-item label="发票类型" prop="invoiceType">
        <el-select v-model="formData.invoiceType" :disabled="isScan" placeholder="请选择发票类型" @change="handleInvoiceTypeChange">
      ...
      <el-form-item label="发票组织" prop="billingOrganization">
        <el-input v-model="formData.billingOrganization" :disabled="isModify" :readonly="isScan" placeholder="开票组织"></el-input>
      </el-form-item>
      <el-form-item label="开票供应商" prop="billingCustomer">
        <el-input v-model="formData.billingCustomer" :disabled="isModify" :readonly="isScan" placeholder="开票客户"></el-input>
      </el-form-item>
      <el-form-item label="供应商打印名称" prop="invoiceHeader">
        <el-input v-model="formData.invoiceHeader" :disabled="isVAT" :readonly="isScan" placeholder="供应商打印名称"></el-input>
      </el-form-item>
      ...
      <el-form-item label="来源单据类型" prop="sourceDocumentType">
        <el-input v-model="formData.sourceDocumentType" disabled :readonly="isScan" placeholder="来源单据类型"></el-input>
      </el-form-item>
      <el-form-item label="税号" prop="taxNo">
        <el-input v-model="formData.taxNo" :disabled="isVAT" :readonly="isScan" placeholder="税号"></el-input>
      </el-form-item>
      <el-form-item label="开户行" prop="openingBank">
        <el-input v-model="formData.openingBank" :disabled="isVAT" :readonly="isScan" placeholder="开户行"></el-input>
      </el-form-item>
      <el-form-item label="账号" prop="accountNumber">
        <el-input v-model="formData.accountNumber" :disabled="isVAT" :readonly="isScan" placeholder="账号"></el-input>
      </el-form-item>
      <el-form-item label="供应商地址" prop="customerAddress" class="col-span-2">
        <el-input v-model="formData.customerAddress" :disabled="isVAT" :readonly="isScan" placeholder="供应商地址"></el-input>
      </el-form-item>
      <el-form-item label="联系方式" prop="contactWay">
        <el-input v-model="formData.contactWay" :disabled="isVAT" :readonly="isScan" placeholder="联系方式"></el-input>
      </el-form-item>
...
import selectionSlider from '@Billing/components/SelectionSlider.vue'
  ...
  data () {
    return {
      ...
      rules: {
        ...
        standardDate: [
          { type: 'date', required: true, message: '请填写发票日期', trigger: 'blur' }
        ],
        billingOrganization: [
  ...
  methods: {
          ...
          this.$http.post('/invoiceInfo/save', this.formData)
            .then(res => {
              if (res.data.status.statusCode === 0) {
                ...
                this.$msgbox({
                  titile: '修改结果',
                  message: res.data.status.statusReason || res.data.result,
                  type: 'error'
                })
              }
              cb && cb()
    ...
    getIntialData () {
      ...
      if (modeParam === 'modify' || modeParam === 'scan') {
        ...
        console.log(this.formData, 'invoicing formData')
        // 如果没有 表体 默认表体为空
