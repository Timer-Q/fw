...
export default {
    ...
    getters: {
        [carConst.CRM_G_GET_NAVTABDATA](state,getters){
            return state.navTabData
        }
    },
    mutations: {
        [carConst.CRM_M_UPDATE_NAVTABDATA](state, payload){
        ...
        [carConst.CRM_M_PUSH_NAVTABDATA] (state, payload) {
            state.navTabData.push(payload)
        },
        [carConst.CRM_M_DELETE_NAVTABDATA] (state, payload) {
            state.navTabData.splice(payload.navIndex, 1)
        }
    },
    actions: {
        // actions的最终功能是修改state，但是它不直接修改state，而是调用mutations
        // async [aboutConst.CRM_A_REQUEST_DATA]({dispatch,commit}) {
