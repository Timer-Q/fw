<template>
    <div class="tms-driver-route-add" ref="OutBox">
                    ...
                    <div class="demo-form-inline">
                        ...
                        >
                            ...
                            >
                                ...
                                <TableColumn width="200" property="carrierName" label="承运商名称"></TableColumn>
                            </Table>
                        </Popover>
                        <Popover
                            ref="popover2"
                        ...
                        <FormItem label="承运商名称" prop="carrierName" >
                            <Input placeholder="请输入承运商名称"
                                   v-popover:popover3
                                   v-model="driverInfo.carrierName"
        ...
        <tooltip name="areaTooltip">
            <el-button size="mini" @click="updateArea">修改</el-button>
...
//            }
        ...
        methods: {
            ...
            async getData() {
                ...
                let params= {
                    "companyId": 1,
                    "deliveryCode": _this.tabMessage.selectedLine.deliveryCode,
                    "driverId": _this.tabMessage.selectedLine.driverId
                }
                let temporaryObj={}
                try {
                    const data = await route.selectRouteMsgById(params)
                    //_this.vehicleDetail = data.data.result
                    console.log(data,'返回的路线------------------')
                    if(data.status==200){
                        temporaryObj=data.data.result
                        _this.driverInfo.addAreatableData=temporaryObj.regionList
                        _this.driverInfo.driverName=temporaryObj.driverName
                        _this.driverInfo.phoneNo=temporaryObj.phoneNo
                        _this.driverInfo.carrierName=temporaryObj.carryName
                        _this.driverInfo.driverId=temporaryObj.driverId
                        _this.driverInfo.canalDescribe=temporaryObj.remarks
                    }
                } catch (e) {
            ...
            async queryDriverInfoByName(value){
                ...
                const params = {
                    ...
                    "carrierName":_this.driverInfo.carrierName,
                ...
                try{
                    ...
                    console.log(data,'司机档案列表driverName')
                    if(data.status==200){
            ...
            async queryDriverInfoByPhone(value){
                ...
                try{
                    const data = await route.getDriverInfoList(params)
                    console.log(data,'司机档案列表phoneNo')
                    if(data.status==200){
            ...
            async queryDriverInfoByCarrier(value){
                ...
                try{
                    const data = await route.getDriverInfoList(params)
                    console.log(data,'司机档案列表carrier')
                    if(data.status==200){
                                    ...
                                    if(_this.areaForm.endNum!==''){
                                        if(_this.areaForm.startNum<_this.areaForm.endNum){
                                            ...
                                            // 修改数据
                                                ...
                                                _this.areaIds=params
                                                console.log(_this.areaIds,'啦啦啦啦啦啦啦啦')
                                                _this.driverInfo.addAreatableData.forEach((item)=>{
                                                    console.log(item,'item------------------861')
                                                    if(commonUtils.cmp(item,_this.areaIds)){
                                                        console.log(item,'item------------------863')
                                                    }
                                                    else{
...
//                }
            ...
            updateArea(){
                ...
                _this.isPush=true
                _this.dialogVisible = true
                _this.areaForm.coverAreaStr=_this.areaIds.coverArea
                _this.areaForm.coverArea.invoiceProvId=_this.areaIds.provinceId
                _this.areaForm.coverArea.invoiceProvName=_this.areaIds.provinceName
                _this.areaForm.coverArea.invoiceCityId=_this.areaIds.cityId
                _this.areaForm.coverArea.invoiceCityName=_this.areaIds.cityName
                _this.areaForm.coverArea.invoiceDistrictId=_this.areaIds.countyId
                _this.areaForm.coverArea.invoiceDistrictName=_this.areaIds.countyName
                _this.areaForm.coverArea.invoiceStreetId=_this.areaIds.townId
                _this.areaForm.coverArea.invoiceStreetName=_this.areaIds.townName
                console.log(_this.areaForm.coverArea,'覆盖区域不对吗')
               // _this.areaForm.coverArea.invoiceAddress=_this.areaIds
               // _this.areaForm.coverArea.invoiceAddressId=_this.areaIds
                _this.areaForm.coverType=_this.areaIds.coverTypeId
                _this.areaForm.coverTypeName=_this.areaIds.coverType
                _this.areaForm.line=_this.areaIds.routeNames
                _this.areaForm.lineId=_this.areaIds.routeIds
                _this.areaForm.roadName=_this.areaIds.roadName
                _this.areaForm.roadNumType=_this.areaIds.numberTypeId
                _this.areaForm.roadNumTypeName=_this.areaIds.numberType
                _this.areaForm.startNum=_this.areaIds.startNum
                _this.areaForm.endNum=_this.areaIds.endNum
                _this.areaForm.siteNum=_this.areaIds.siteNum
            },
            ...
            async saveData(){
                ...
                if(isOk){
                    let params={
                        "belongCarry": 2,// 承运商ID
                        "carryName":_this.driverInfo.carryName,// 承运商名称
                        "companyId": 1,
                        "createUser": 2,
                        "id":_this.tabMessage.selectedLine.id,
                        "driverId": _this.driverInfo.driverId,
                        "driverName": _this.driverInfo.driverName,
                        "phoneNo": _this.driverInfo.phoneNo,
                    ...
                    try{
                        const data = await route.updateSave(params)
                        if(data.status==200){
                            if(data.data.result){
                                _this.$customMessage("修改成功","success")
                            }
                            else{
                                _this.$customMessage(data.data.status.statusReason,"error")
                            }
                            console.log(data,'成功了吗')
                        }
                        else{
                            _this.$customMessage("修改失败","error")
                        }
                    }
                    catch(e){
                        _this.$customMessage("修改失败","error")
