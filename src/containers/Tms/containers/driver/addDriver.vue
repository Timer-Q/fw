<template>
    <div ref="OutBox">
        <Form :inline="true" ref="driverInfo" :rules="ruleCard" :model="driverInfo" label-width="100px">
            ...
            <div>
                    ...
                    <div>
                        <div class="table-row-select">
                            <FormItem label="司机编号" prop="driverCode" required>
                                <Input v-model="driverInfo.driverCode" placeholder="自动编码" :disabled="true"></Input>
                            </FormItem>
                        </div>
                        <div class="table-row-select">
                            <FormItem label="司机姓名" prop="driverName" required>
                                <Input v-model="driverInfo.driverName"></Input>
                                <Checkbox v-model="driverInfo.employee" @change="setEmployee">公司名称</Checkbox>
                            </FormItem>
                        </div>
                        <div class="table-row-select">
                            <FormItem label="承运商名称" prop="belongCarry">
                                <Select placeholder="请选择" v-model="driverInfo.belongCarry" @change="setCarryName"
                                        :disabled="aviable">
                                    <Option
                    ...
                    <div>
                        <div class="table-row-select">
                            <FormItem label="身份证号" prop="identityCard">
                                <Input v-model="driverInfo.identityCard" @change=""></Input>
                            </FormItem>
                        </div>
                        <div class="table-row-select">
                            <FormItem label="手机号码" prop="phoneNo">
                                <Input v-model="driverInfo.phoneNo" @change="" placeholder="电话号码/手机号码必须填一项"></Input>
                            </FormItem>
                        </div>
                        <div class="table-row-select">
                            <FormItem label="电话号码">
                    ...
                    <div>
                        <div class="table-row-select">
                            <FormItem label="初次领证日期" prop="firstLicenseDatetime">
                                <el-date-picker type="date" @change=""
                                                v-model="driverInfo.firstLicenseDatetime"></el-date-picker>
                            </FormItem>
                        </div>
                        <div class="table-row-select">
                            <FormItem label="年检、换证日期" prop="yearCheckDatetime">
                                <el-date-picker type="date" @change=""
                                                v-model="driverInfo.yearCheckDatetime"></el-date-picker>
                            </FormItem>
                        </div>
                        <div class="table-row-select">
                            <FormItem label="准驾车型">
                                <Select placeholder="请选择" v-model="driverInfo.driveType">
                                    <Option
                                        v-for="item in zjcx"
                                        :key="item.itemCode"
                                        :label="item.itemName"
                        ...
                        <div class="table-row-select">
                            <FormItem label="驾驶证号" prop="driverNo" required>
                                <Input v-model="driverInfo.driverNo" @change=""></Input>
                            </FormItem>
                        </div>
                    </div>
                    <div>
                        <div class="table-row-select">
                            <FormItem label="常住地址">
                                ...
                                <Input v-model="driverInfo.address " @change="" placeholder="详细地址"></Input>
                            </FormItem>
                        </div>
                ...
                <div>
                    <div class="table-row-select">
                        <FormItem label="备注">
                            <el-input v-model="driverInfo.remarks" @change=""></el-input>
                        </FormItem>
                    </div>
                </div>
            ...
            <div class="">
                <Tabs v-model="activeName" type="card" @tab-click="">
                    ...
                    <Table
                        :data="driverInfo.baseDriverBankDTOs"
            ...
            <div>
                ...
                <div class="table-row-select">
                    <FormItem label="制单人">
                        <Input v-model="driverInfo.createUser" @change=""></Input>
                    </FormItem>
                </div>
                <div class="table-row-select">
                    <FormItem label="制单时间">
                        <el-date-picker type="date" @change=""
                                        v-model="driverInfo.createTime"></el-date-picker>
                    </FormItem>
                </div>
            </div>
        </Form>
        ...
        <div class="success" v-show="haveVehicle">用户姓名已存
            在！或者驾驶证号已存在，请核对！
        </div>
    </div>
</template>
...
//            console.log('当前时间---',(new Date()).getTime())
        ...
        methods: {
            addArea(flag) {
                ...
                let _this = this, count = 0, countDefault = 0
                ...
                _this.$refs['bankForm'].validate((valid) => {
                    if (valid) {
                        ...
                        console.log('----', params)
                        //校验银行账号是否存在列表中
                        _this.driverInfo.baseDriverBankDTOs.forEach((item, index) => {
                            if (params.bankAccount == item.bankAccount) {
                                count = 1
                            }
                            console.log('--item.idafault-', item.idafault)


                            if (params.idafault == item.idafault && item.idafault == 'true') {
                        ...
                        if (countDefault == 1) {
                            _this.$customMessage("只能默认一个银行帐号，请核对！！", 'warning')
                            return false
                        }
                        _this.driverInfo.baseDriverBankDTOs.push(params)
            ...
            setEmployee(value) {
                if (this.driverInfo.employee) {
                    this.aviable = true
                    this.driverInfo.belongCarry = ''
                    this.driverInfo.carrierName = ''

                } else {

                    this.aviable = false
                }
                console.log(this.driverInfo.employee)
            ...
            removeArea(index) {
                ...
                if (_this.selectIds.length) {
                    ...
                    _this.driverInfo.baseDriverBankDTOs = selectRecordsTable
            ...
            async saveChannel(flag) {
                ...
                if (ifSave) {
                    try {
                        ...
                        _this.driverInfo.attachmentUrl = strs//图片路径
                        _this.driverInfo.headUrl = strsHead//头像路径

                        let telNos = _this.qu + '-' + _this.contact + '-' + _this.fen//拼接电话号码
                        _this.driverInfo.telNo = telNos
                        //判断电话号码或者手机号码是否为空
                        console.log('telNo', telNos)
                        if (_this.driverInfo.phoneNo == '' && _this.contact == '') {
                            this.$customMessage("手机号和电话号码必须必填一项", "error")
                            return false
                        }
                        //赋值省市区街道
                        if (_this.coverArea != '') {
                            _this.driverInfo.province = _this.coverArea.invoiceProvId
                            _this.driverInfo.city = _this.coverArea.invoiceCityId
                            _this.driverInfo.county = _this.coverArea.invoiceDistrictId
                            _this.driverInfo.street = _this.coverArea.invoiceStreetId
                        }

                        const message = _this.driverInfo
                        const data = await driverService.add(message)
                        if (data.data.result == 1) {
                            this.$customMessage("保存成功！", "success")
                            if (flag == 'new') {
                                //this.saveForm = _this.clearForm
                                _this.driverInfo = _this.clearDriverInfo

                            } else {
                                //  _this.$root.bus.$emit("removeChangeTab", "tms_driverManage_add")
//                                _this.close()
            ...
            ,
            cancel() {
                this.$emit("removeChangeTab", "tms_driverManage_add")
            }
            ,
            closeTab() {
...
//                alert(eValue);
                ...
                for (var i = 0, vlen = _this.carryList.length; i < vlen; i++) {
                    if (_this.carryList[i].logisticsId == value) {
                        ...
                        this.driverInfo.carrierName = _this.carryList[i].custName
...
//                this.dialogVisible = true
            ...
            setHeadUrl(res, file) {
                if (res.status.statusCode == 0) {
                    ...
                    this.imageUrl = 'http://tms-1254060329.cosbj.myqcloud.com' + res.result
                }
                else {
                    this.$customMessage("上传失败,请检查图片大小重新上传！", "error")
//                    if (fileList.length == 1) {
