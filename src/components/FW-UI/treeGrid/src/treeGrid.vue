...
<script>
    ...
    export default {
        ...
        mounted(){
            ...
            //
            //     setTimeout(()=>{
            //         util.clog('owner el:',this.getOwner().$el)
        ...
        methods:{
            handleExpandAfterDataRendered(result,commit=true,checkedChildsIfParentChecked=true){
                let {allCheckedIds}=result
                util.clog('handleExpandAfterDataRendered args:',arguments)
                let t=Date.now(),res
                if(!this.isVirtualTree){
                    ...
                    // 展开所有1级节点
                        ...
                        this.commitTableStoreData(result.eachedData)
                    }
                }
                util.clog('真实tree首次展开用时：',Date.now()-t,res)
                return res
            ...
            commitTableStoreData(data){
                if(data.length>0){
                    ...
                    // this.getOwner().data.push(data[0])
                    // this.getOwner().data.splice(0,0,data)
                    util.clog('get owner:',this.getOwner())
                    let owner=this.getOwner()

                    owner.store.commit('setData',data)
                    this.$nextTick(()=>{

                        owner.$el.parentNode.classList.add('tree-table-parent-with-data')
                        // alert(owner.$el.classList.contains('tree-table-parent-with-data'))
                        //
                        // setTimeout(()=>{
                        //     alert(owner.$el.classList.contains('tree-table-parent-with-data'))
                        // },2000)
            ...
            getLastDescendantChildIndex(index,deepth,tree){
                let treeLen=tree.length
                index+=1

                for(var i=index;i<treeLen;i++){
            ...
            insertRows(index,rows,isSibling){
                ...
                let realData=this.getRealData(),
                    ...
                    // virtualTreeLen=this.initialTreeCopier.length,
                    row=realData[index],
                ...
                let id=row[this.treeKey],
                    ...
                    virtualParentNode=!isSibling?(this.initialTreeCopier[index]):this.getParentNode(this.initialTreeCopier,shadowRow.row),

                    shadowRowIndex=this.getLastDescendantChildIndex(shadowRow.index,rowDeepth,this.initialTreeCopier)
                //     shadowRowIndex=shadowRow.index+1
                //
                // for(var i=shadowRowIndex;i<virtualTreeLen;i++){
                //     if(this.initialTreeCopier[i].$extra.deepth==rowDeepth){
                //
                //         break
                //     }else{
                //         shadowRowIndex++
                ...
                // util.clog('real data len:',realData.length)
                ...
                // util.clog('fucku realData--:',realData,realData.length,JSON.stringify(realData))
                ...
                // util.clog('this.initialTreeCopier---:',this.initialTreeCopier)
                //
                // return {
                //     expandedTree:expandedTree,
                //     realParentNode:realParentNode,
                //     virtualParentNode:virtualParentNode
                // }
            ...
            filterRows(conFun,GET_TYPE='ROW',commit=true){
                ...
                while(i<this.initialTreeCopier.length){
                    ...
                    if(conFun(item)){
                        ...
                        util.clog('descendantChilds---:',descendantChilds)
                ...
                util.clog('fucku new data:',newData)
            ...
            refreshTreeTable(data,isNestedStructure=true){
                ...
                util.clog('refreshTreeTable-- data:',data)

                this.allExistTreeKeys=[]

                this.updateTreeTable(isNestedStructure,data,1,(result)=>{
                    util.clog('fukcu result---:',result)
                    this.initialTreeCopier=util.copy(result.expandedTree)

                    this.handleExpandAfterDataRendered(result)
                })
                ...
                //
                // this.allExistTreeKeys=[]
                // util.clog('helloooooooooo',data)
                // let expandedTree
                //
                // if(isNestedStructure){
                //     expandedTree=this.updateTreeTable(true,data,1,function(options){
                //         util.clog('fucku options:',options)
                ...
                //
                // }
                // // this.commitTableStoreData(expandedTree)
                //
                //
                //
            ...
            // 参数指定是否从源数据更新
            updateTreeTable(isNestedStructure=true,insertRows=null,beginDeepth=1,cb){
                cb=cb||function(){}
                let self=this,
                    eachedData,
                    maxDeepth=1,
                ...
                //
                //
                // util.clog('is from source:',isNestedStructure)
                ...
                let expandedTree=isNestedStructure?(this.expandTree(maxDeepth,insertRows,insertRows==null?false:true)):insertRows
            ...
            syncChildNodesCheckedStatus(row){
                ...
                let virtualTreeDescendantChilds=util.getDescendantChilds(id,data,this.treeKey,'ROW'),
            ...
            toggleExpandChildsByIndex(index,ifSetData,fromListData,isInsertRowMutation=false){
                ...
                if(data[index].$extra.expanded){
                    ...
                    util.clog('this.initialTreeCopier.length--:',this.initialTreeCopier.length)
                    ...
                    if(this.initialTreeCopier.length==0){
                        ...
                        if(isInsertRowMutation){
                            ...
                            // getChildsByParentId(pid,fromData,resType='ROW')
                            allChildNodes=this.getChildsByParentId(row[this.treeKey],this.initialTreeCopier)
